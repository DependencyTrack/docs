...
The polling interval is configurable via [`dt.notification.outbox-relay.poll-interval-ms`](../../../reference/configuration/properties.md#dtnotificationoutbox-relaypoll-interval-ms),
...
Refer to the [publishers](../../../reference/notifications/publishers.md) documentation for details.
...
Activities can signal retryable failures (for example, rate limiting or temporary unavailability).
The durable execution engine transparently handles retries with backoff. Non-retryable failures are terminal
