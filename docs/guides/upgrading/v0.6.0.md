...
* The required PostgreSQL version has been raised from 11 to 13 ([hyades/#1724]).
...
conflicting usernames get their values suffixed with `-CONFLICT-LDAP` and
`-CONFLICT-OIDC` respectively. Affected users cannot authenticate.
Administrators are expected to resolve this by removing users or renaming them as desired.
Note that this is an edge case and should not affect most deployments.
...
Only historic data falling within the configured retention duration gets migrated.

* Database migrations have been integrated into the broader concept of *initialization tasks*.
As a result, the following configuration properties were renamed:
